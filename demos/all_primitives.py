...
def main():
    ...
    ball = prim.Ball(name="ball", radius=.02, geom='HD', phys=False)
    ball.create().set_pos(Point3(-.15, 0, .1))
    ball.attach_to(app.models, app.world)

    box = prim.Box(name="box", extents=[.05, .03, .01], geom='HD', phys=False)
    box.create().set_pos(Point3(-.05, 0, .1))
    box.attach_to(app.models, app.world)

    cylinder = prim.Cylinder(name="cylinder", extents=[.01, .04], geom='HD',
                             phys=False)
    ...
    goblet = prim.Goblet(name="goblet", extents=[.05, .025, .015], geom='HD',
                         phys=False)
    goblet.create().set_pos(Point3(-.15, 0, -.025))
    goblet.attach_to(app.models, app.world)

    track = prim.Track(name="track", extents=[.06, .02, .006, .002], geom='HD',
                       phys=False)
    ...
    rp_cyl = prim.Cylinder(
        name="rp-cyl", extents=[.01, .02], geom='HD', phys=False, mass=1
    )
    rp_cyl.create().set_pos(Point3(0, 0, -.05))
    rp_cube = prim.Box(
        name="rp-box", extents=[.02, .02, .02], geom='HD', phys=False
    )
    ...
    rpp_cyl = prim.Cylinder(
        name="rpp-cyl", extents=[.01, .02], geom='HD', phys=False, mass=1
    )
    rpp_cyl.create().set_pos(Point3(0, 0, -.05))
    rpp_cube = prim.Box(
        name="rpp-box", extents=[.05, .02, .005], geom='HD', phys=False
    )
